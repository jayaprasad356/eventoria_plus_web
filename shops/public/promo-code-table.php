...
<section class="content-header">
  <h1>Coupon Codes /<small><a href="home.php"><i class="fa fa-home"></i> Home</a></small></h1>
</section>
<!-- Main content -->
<section class="content">
  ...
  <div class="row">
    <div class="col-md-6">
      <!-- <?php if ($permissions['coupon_codes']['create'] == 0) { ?>
        <div class="alert alert-danger">You have no permission to create coupon code.</div>
      <?php } ?> -->
      <!-- general form elements -->
      <div class="box box-primary">
        <div class="box-header with-border">
          <!-- <h3 class="box-title">Manage Coupon Code</h3> -->

        </div><!-- /.box-header -->
        <!-- form start -->
        <form method="post" id="add_form" action="public/db-operation.php">
          <input type="hidden" id="add_coupon_code" name="add_coupon_code" required="" value="1" aria-required="true">
          <div class="box-body">
      ...
      <div class="box">
        <div class="box-header">
          ...
          <div class="box-body table-responsive">
            <table class="table table-hover" data-toggle="table" id="coupon-codes" data-url="get-bootstrap-table-data.php?table=coupon-codes" data-page-list="[5, 10, 20, 50, 100, 200]" data-show-refresh="true" data-show-columns="true" data-side-pagination="server" data-pagination="true" data-search="true" data-trim-on-search="false" data-sort-name="id" data-sort-order="desc">
              <thead>
                <tr>
  ...
  <div class="modal fade" id='editcouponCodeModal' tabindex="1" role="dialog" aria-labelledby="myLargeModalLabel">
    <div class="modal-dialog modal-md" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <!-- <?php if ($permissions['coupon_codes']['update'] == 0) { ?>
            <div class="alert alert-danger">You have no permission to update coupon code.</div>
          <?php } ?> -->
          <div class="box-body">
            <form id="update_form" method="POST" action="public/db-operation.php" data-parsley-validate class="form-horizontal form-label-left">
              ...
              <input type='hidden' name="update_coupon_code" id="update_coupon_code" value='1' />
              <div class="form-group">
                <label for="">coupon Code</label>
                <input type="text" class="form-control" name="update_coupon" id="update_coupon">
              </div>
              <div class="form-group">
                <label for="">Message</label>
              ...
              <div class="form-group">
                ...
                <select id="update_category" name="update_category" class='form-control'>
                  <option value="0">All</option>
                            ...
                            foreach ($result as $value) {
                            ?>
								<option value='<?= $value['id'] ?>' <?= $value['id']==['update_category'] ? 'selected="selected"' : '';?>><?= $value['name'] ?></option>
                        <?php } ?>
                </select>
            </div>
              <div class="form-group">
...
<script>
  ...
  $('#add_form').on('submit', function(e) {
    ...
    if ($("#add_form").validate().form()) {
      if (confirm('Are you sure?Want to Add coupon Code')) {
        $.ajax({
          type: 'POST',
          ...
          success: function(result) {
            ...
            $('#add_form')[0].reset();
            $('#coupon-codes').bootstrapTable('refresh');
          }
        });
      }
    }
  ...
  window.actionEvents = {
    ...
    'click .edit-coupon-code': function(e, value, row, index) {
      $("input[name=status][value=1]").prop('checked', true);
      if ($(row.status).text() == 'Deactive')
        $("input[name=status][value=0]").prop('checked', true);
      $('#coupon_code_id').val(row.id);
      $('#update_coupon').val(row.coupon_code);
      $('#update_message').val(row.message);
      $('#update_category').val(row.category_id);
  ...
  $(document).on('click', '.delete-coupon-code', function() {
    if (confirm('Are you sure? Want to delete coupon code.')) {
      id = $(this).data("id");
      $.ajax({
        url: 'public/db-operation.php',
        type: "get",
        data: 'id=' + id + '&delete_coupon_code=1',
        success: function(result) {
          if (result == 0) {
            $('#coupon-codes').bootstrapTable('refresh');
          }
          if (result == 2) {
            alert('You have no permission to delete coupon code');
          }
          if (result == 1) {
            alert('Error! coupon code could not be deleted.');
          }
        }
      });
    }
